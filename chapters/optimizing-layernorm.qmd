...
date: 2026-09-17
image: ../assets/layernorm/layernorm-overview.svg
---
...
![](../assets/layernorm/layernorm-overview.svg){fig-alt="One row of features is summarized by a mean and variance, standardized, and then transformed by the learned gamma and beta vectors." width=100%}
...
::: {.callout-note title="What about the cache?" collapse="true"}
...
![](../assets/layernorm/layernorm-warp-row.svg){fig-alt="A four-lane teaching example in which neighboring lanes read neighboring LayerNorm columns and each lane builds a local Welford state before the states are merged." width=100%}
...
::: {.callout-note title="Where does the correction term come from?" collapse="true"}
...
![](../assets/layernorm/welford-merge.svg){fig-alt="Two Welford states, each containing a count, mean, and M2, are combined with a correction term based on the distance between their means." width=100%}
...
![](../assets/layernorm/layernorm-memory-passes.svg){fig-alt="The naive LayerNorm kernel reads the input three times, the Welford warp kernel reads it twice, and the shared-memory block kernel reads it once and reuses an on-chip row copy." width=100%}
...
::: {.callout-note title="When is one warp not enough?" collapse="true"}
...
::: {.callout-note title="Which version is faster?" collapse="true"}
